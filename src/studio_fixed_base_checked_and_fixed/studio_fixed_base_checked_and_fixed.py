    ...
    with right_panel:
        ...
        with st.expander("Metadatos estructurales (episodio / bloque / escena)", expanded=False):
            ...
            with c5:
                ...
                scene.attrs["continuity_notes"] = st.text_area("scene.continuity_notes", value=scene.attrs.get("continuity_notes", ""), key=f"struct_scene_continuity_{scene_key}", height=90, help=field_help("scene.continuity_notes"))
        ...
        beat_head1, beat_head2 = st.columns([6, 2])
        with beat_head1:
            st.markdown("##### Texto del beat", help=field_help("Texto del beat"))
        with beat_head2:
            auto_fill_scene_clicked = tip_button(
                "🪄 Analizar beats de la escena",
                key=f"struct_autofill_scene_{scene_key}",
                help="Analiza el texto de todos los beats de la escena y rellena automáticamente la duración y los metadatos audiovisuales más plausibles.",
        ...
        if auto_fill_scene_clicked:
            analyzed_count = 0
            for scene_beat in scene.beats:
                apply_inferred_beat_attrs(scene_beat, scene_beat.text, catalogs)
                scene_beat_key = scene_beat.attrs.get("id") or stable_node_key(scene_beat)
        ...
        with c_lock2:
            beat.attrs["continuity_notes"] = st.text_area("beat.continuity_notes", value=beat.attrs.get("continuity_notes", scene.attrs.get("continuity_notes", "")), key=f"struct_beat_continuity_{beat_key}", height=90, help=field_help("beat.continuity_notes"))
