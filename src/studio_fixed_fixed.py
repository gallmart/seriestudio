...
def render_scene_beats_sidebar(scene: Scene, block: Block, scene_idx: int, episodes: List[Episode], key_prefix: str):
    ...
    for bt in scene.beats:
        try:
            total_scene_duration += float(bt.attrs.get("duration", 0) or 0)
        except Exception:
            pass
    ...
    minutes = int(total_scene_duration // 60)
    seconds = total_scene_duration - (minutes * 60)
    st.caption(f"Duración total de la escena: {total_scene_duration:.1f} s ({minutes:02d}:{seconds:04.1f})")
...
def tab_structure_editor(root: Path):
    ...
    with right_panel:
        ...
        with ref1:
            reference_mode_options = ["AUTO", "START_IMAGE", "START_END_IMAGES"]
            current_reference_mode = beat.attrs.get("reference_mode", "AUTO")
            if current_reference_mode not in reference_mode_options:
                current_reference_mode = "AUTO"

            beat.attrs["reference_mode"] = st.selectbox(
                "beat.reference_mode",
                reference_mode_options,
                index=reference_mode_options.index(current_reference_mode),
                key=f"struct_beat_reference_mode_{beat_key}",
                help="Controls whether this beat uses no explicit image reference, only a start image, or both start and end images for video generation.",
            )

            visual_transition_options = ["NONE", "SOFT_TRANSITION", "HARD_CUT", "MORPH", "DISSOLVE"]
            current_visual_transition = beat.attrs.get("visual_transition", "NONE")
            if current_visual_transition not in visual_transition_options:
                current_visual_transition = "NONE"

            beat.attrs["visual_transition"] = st.selectbox(
                "beat.visual_transition",
                visual_transition_options,
                index=visual_transition_options.index(current_visual_transition),
                key=f"struct_beat_visual_transition_{beat_key}",
                help="Visual transition hint for the video model.",
            )

            beat.attrs["ref_image_start"] = st.text_input(
                "beat.ref_image_start",
                value=beat.attrs.get("ref_image_start", ""),
                key=f"struct_beat_ref_image_start_{beat_key}",
                help="Optional path to an explicit start image for this beat.",
            )

        with ref2:
            beat.attrs["ref_image_end"] = st.text_input(
                "beat.ref_image_end",
                value=beat.attrs.get("ref_image_end", ""),
                key=f"struct_beat_ref_image_end_{beat_key}",
                help="Optional path to an explicit end image for this beat.",
            )
            ...
            audio_transition_options = ["CUT", "FADE", "CROSSFADE"]
            current_audio_transition = beat.attrs.get("audio_transition", "CUT")
            if current_audio_transition not in audio_transition_options:
                current_audio_transition = "CUT"

            beat.attrs["audio_transition"] = st.selectbox(
                "beat.audio_transition",
                audio_transition_options,
                index=audio_transition_options.index(current_audio_transition),
                key=f"struct_beat_audio_transition_{beat_key}",
                help="Montage hint for audio continuity between beats.",
            )
        ...
        if target_level == "beat":
            fields = ["kind", "shot", "framing", "camera", "focus", "transition", "duration", "audio", "video", "visual_style", "music_tag", "bgm_level", "sfx_level", "speech_rate", "voice_intensity", "pause_before", "pause_after", "voice_id", "continuity_notes", "lock_location", "lock_props", "lock_voice", "lock_visual_style"]
        ...
        if tip_button("Aplicar de aquí para adelante", key="struct_apply_propagation"):
            if target_level == "beat":
                propagate_beat_attr(episodes, episode_idx, block_idx, scene_idx, beat_idx, field_to_propagate, current_value_to_propagate, overwrite_existing, selected_scope)
            elif target_level == "scene":
                propagate_scene_attr(episodes, episode_idx, block_idx, scene_idx, field_to_propagate, current_value_to_propagate, overwrite_existing, selected_scope)
            else:
...
def tab_tag_editor(root: Path):
    ...
    with right:
        ...
        beat.attrs["speech_rate"] = f"{st.slider('beat.speech_rate', 0.85, 1.10, speech_rate_val, 0.01, key='tag_beat_speech_rate', help=BEAT_META_HELP['beat.speech_rate']):.2f}"
        beat.attrs["pause_before"] = f"{st.slider('beat.pause_before', 0.0, 0.5, pause_before_val, 0.01, key='tag_beat_pause_before', help=BEAT_META_HELP['beat.pause_before']):.2f}"
        beat.attrs["pause_after"] = f"{st.slider('beat.pause_after', 0.0, 0.5, pause_after_val, 0.01, key='tag_beat_pause_after', help=BEAT_META_HELP['beat.pause_after']):.2f}"
...
def tab_script_editor(root: Path):
    ...
    if editor_section == "Segmentación":
        tab_segmentation(root)
    elif editor_section == "Estructura":
